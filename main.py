...
def run_job_ingestion_process(aws_job_id: str, days) -> None:
    try:
        ...
        start_time = datetime.datetime.now().time().strftime("%H:%M:%S")
        ingestion = Ingestion()
        record_list = ingestion.process(days)
        store_item = Store()
        store_item.store_transaction(
            item=record_list[0:1],
            job_id=aws_job_id,
            PK="timezoneloc",
...
def main(overwrite, days, env, aws_job_id):
    start = time.time()
    ...
    job_function = function_dict.get("ingestion_process_job")
