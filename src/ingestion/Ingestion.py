...
class Ingestion:
        ...
        try:
            ...
            for dic in lst_loc:
                ...
                for day in range(0,int(days)):
                    ...
                    response = requests.get(f"{os.environ['END_POINT']}{lat}&lon={lon}&dt={time}&appid={API_KEY}")
    ...
    def clean_duplicates(self,df):
        ...
        new_df["locationtime"] = new_df.dt.apply(lambda x: datetime.utcfromtimestamp(x).strftime('%Y-%m-%d %H:%M:%S'))
    ...
    def combine_duplicates(self,df, set_columns):
        for value in set_columns:
            df = pd.concat([
                ...
                pd.DataFrame([self.flatten_json(nested_json=x,column_name=value) for x in df[value]])
                ],axis=1)
        return df
